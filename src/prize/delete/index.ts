...
import {
  AdventureApp, generate_telemetry, Prizes, response,
} from '/opt/nodejs';
...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  if(!event.pathParameters)
    return response(500, "No path parameters. This should never happen!");
  if(!event.headers.Authorization)
    return response(401, "Need authentication to delete a prize");
