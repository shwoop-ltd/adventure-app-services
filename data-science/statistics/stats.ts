...
const answers: [{ id: string; solution: string }] = JSON.parse(
  fs.readFileSync('C:/Users/russe/Documents/GitHub/adventure-app-services/scripts/Statistics/challenge_answers.json', 'utf-8'),
);
...
function outputChallengeCompleteTimesByUser() {
  ...
  const min = Math.min(
    ...Object.keys(user_challenge_complete_time)
      .map((id) => Math.min(...user_challenge_complete_time[id]
        .map((challenge) => challenge.time))),
  );
  const max = Math.max(
    ...Object.keys(user_challenge_complete_time)
      .map((id) => Math.max(...user_challenge_complete_time[id]
        .map((challenge) => challenge.time))),
  );
