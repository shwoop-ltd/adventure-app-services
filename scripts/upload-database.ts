...
const { filter, profile, db, stage } = args
...
async function run() {
  ...
  let items = (await Promise.all(input_files.map((filename) => fs.readFile(filename, { encoding: 'utf-8' })))).reduce(
    (prev, cur) => prev.concat(JSON.parse(cur)),
    [] as Item[]
  )
